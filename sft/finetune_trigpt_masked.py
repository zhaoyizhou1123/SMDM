...
from lit_gpt.trimodel import TriGPT, Block
...
def parse_args():
    ...
    parse.add_argument('--pretrain_path', type=str, default = "", help='pretrain ckpt path')
...
def setup(
    ...
    global out_dir
    hp_name = f'trigpt-{args.model}M-scratch-{args.order}'
    if args.r2l:
        hp_name += '-r2l'
    if args.postfix != '':
...
def train(fabric, state, train_dataloader, monitor, resume):
    ...
    with torch.device("meta"):
        meta_model = TriGPT(model.config)

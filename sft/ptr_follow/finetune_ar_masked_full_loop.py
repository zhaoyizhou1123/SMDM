...
# Combines:
#   - sft/finetune_ar_masked.py  (masked forward process)
#   - sft/ptr_follow/finetune_ar_custom_full_loop.py  (full-loop / validation structure)
#   - sft/ptr_follow/eval_double_ar_iterative_unmask.py  (dual-model evaluation)
...
# ---------------------------------------------------------------------------
# Masked forward process (from sft/finetune_ar_masked.py)
# ---------------------------------------------------------------------------
...
def double_ar_sample_iterative_unmask(model_l2r, model_r2l, prompt, response_length=16, device='cuda'):
    """Iterative unmasking using two AR models (L2R and R2L).

    Starts with all response positions masked (token id=0). At each step, both
    models score every still-masked position; the single most-confident position
    is permanently filled. Repeats until all positions are filled.
    """
    MASK_TOKEN_ID = 0
    ...
    for _ in range(response_length):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            r2l_logits = model_r2l(flipped_x)[:, -(response_length+1):-1].flip(dims=[1])  # (B, resp_len, vocab); same shift correction
...
def setup(
    ...
    global out_dir
    hp_name = f'arm-{args.model}M-masked-{args.order}-double-corrected'
    if args.postfix != '':

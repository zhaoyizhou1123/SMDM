...
def setup(
    ...
    global out_dir
    hp_name = f'arm-{args.model}M-masked-v2-{args.order}-double'
    if args.postfix != '':
...
def train(fabric, state, train_dl_l2r, train_dl_r2l, val_dataloader, monitor, resume):
    ...
    for l2r_data, r2l_data in zip(infinite(train_dl_l2r), infinite(train_dl_r2l)):
        ...
        # ---- R2L forward / backward (standard AR training, no masking) -----
        ...
        with fabric.no_backward_sync(r2l_model, enabled=is_accumulating):
            logits_r2l = r2l_model(input_ids_r2l)
            # Standard AR: predict each response token from its causal prefix.
            # logit at position (prompt_len - 1) predicts token at prompt_len, etc.
            b, seq_len, vocab = logits_r2l.shape
            tmp = torch.arange(seq_len, device=input_ids_r2l.device).unsqueeze(0).expand(b, -1)
            resp_start = prompt_len_r2l.unsqueeze(1)   # first response token position
            resp_end   = length_r2l.unsqueeze(1)       # one past last response token
            # logits at [resp_start-1 .. resp_end-2] predict tokens at [resp_start .. resp_end-1]
            logits_r2l_flat = logits_r2l[(tmp >= resp_start - 1) & (tmp < resp_end - 1)]
            targets_r2l     = input_ids_r2l[(tmp >= resp_start) & (tmp < resp_end)]
            loss_r2l = loss_func(logits_r2l_flat, targets_r2l)

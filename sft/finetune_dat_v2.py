...
from lit_gpt.datmodel_v2 import DualStreamGPTV2, Block, FusedDualBlock
...
def setup(
    ...
    global out_dir
    hp_name = f'dat-{args.model}M-v2-{args.order}'
    if args.r2l:
        hp_name += '-r2l'
    if args.postfix != '':
    ...
    if devices > 1:
        if tpu:
            ...
            strategy = FSDPStrategy(
                auto_wrap_policy={Block, FusedDualBlock},
...
def main(fabric, pretrain_path, resume):
    ...
    train(fabric, state, train_dataloader, monitor, resume, eos_token_id=tokenizer.eos_token_id)
    fabric.print(f"Training time: {(time.perf_counter()-train_time):.2f}s")
    if fabric.device.type == "cuda":
        fabric.print(f"Memory used: {torch.cuda.max_memory_allocated() / 1e9:.02f} GB")


def train(fabric, state, train_dataloader, monitor, resume, eos_token_id):
    model = state["model"]
    optimizer = state["optimizer"]

    with torch.device("meta"):
        meta_model = DualStreamGPTV2(model.config)
    ...
    for train_data in train_dataloader_:
        ...
        eos_ids = torch.full((input_ids.size(0), 1), eos_token_id, dtype=input_ids.dtype, device=input_ids.device)
        input_ids = torch.cat((input_ids, eos_ids), dim=-1)
        ...
        with fabric.no_backward_sync(model, enabled=is_accumulating):
            ...
            logits_index = (temp_tensor >= (prompt_length).unsqueeze(1)) & (temp_tensor < (length).unsqueeze(1)) 
            ...
            targets = input_ids[logits_index]
            # print(targets[0])

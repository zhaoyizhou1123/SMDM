# Double masked ARM as diffusion model
...
from eval.gen_model_answer import masked_ar_sample, generate
...
def get_masked_ar_sample(args, question, model_l2r, model_r2l, tokenizer):
    ...
    answer_ids = masked_ar_sample(model_l2r,
                             model_r2l,
                             tokenizer,
                             question_ids,
                             alg='low_confidence',
    ...
    #     answer_ids = generate(model_r2l, tokenizer, question_ids, args.length)
...
if __name__ == "__main__":
    ...
    model_l2r = GPT(config).to(device)
    model_l2r.load_state_dict(load_file(args.ckpt_path_l2r))

    model_r2l = GPT(config).to(device)
    model_r2l.load_state_dict(load_file(args.ckpt_path_r2l))
    ...
    for i in range(iter):
        ...
        preds = get_masked_ar_sample(args, questions, model_l2r, model_r2l, tokenizer)

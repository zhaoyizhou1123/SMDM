...
def parse_args():
    ...
    parse.add_argument('--r2l', action='store_true', help='whether to use right to left training')
    parse.add_argument('--weighted_loss', action='store_true', help='whether to use weighted loss')
...
def forward_process_mad(batch, total_dim=32000, eps=1e-3):
    b, l = batch.shape
    ...
    n_mask = torch.randint(low=0, high=l, size=(b,), device=batch.device)
    n_mask = n_mask.unsqueeze(1).repeat(1, l)
    ...
    mask_indices = (pos_indices < n_mask)
    noisy_batch = torch.where(mask_indices, total_dim, batch)
    return noisy_batch, mask_indices, n_mask
...
def setup(
    ...
    if args.r2l:
        ...
        postfix = 'l2r'
    hp_name = f'mad-{postfix}-{args.model}M-{args.flops}'
    out_dir = Path('workdir/pretrain') / hp_name
    wandb_logger = WandbLogger(name=f'{hp_name}-mc', save_dir=out_dir, project='pretrain-smdm')
...
def train(fabric, state, train_dataloader, val_dataloader, monitor, resume):
    ...
    for  train_data in train_dataloader:
        ...
        noisy_input, mask_indices, n_mask = forward_process_mad(input_ids)
        ...
        with fabric.no_backward_sync(model, enabled=is_accumulating):
            ...
            # I am not sure whether dividing by n_mask is correct or not.
            loss = loss_func(logits[mask_indices], input_ids[mask_indices])
            if args.weighted_loss:
                loss = loss / (n_mask[mask_indices]+1)

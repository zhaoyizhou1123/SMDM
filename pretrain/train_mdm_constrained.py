...
def parse_args():
    ...
    parse.add_argument('--block_size', type=int, default=2048, help='context length')
    parse.add_argument('--micro_batch_size', type=int, default=None, help='micro batch size. If not set, will be determined by the model size')
...
if args.micro_batch_size is not None:
    ...
    micro_batch_size = 4
max_step = int(args.flops * 1e12 / (6 * model_para_config[f'{args.model}'] * global_batch_size * args.block_size) / args.nodes_num)
warmup_steps = int(max_step / 100) if int(max_step / 100) > 100 else 100
log_step_interval = 10
# eval_iters = int(100 * 1024 / global_batch_size)
eval_iters = 10
save_step_interval = 1000
eval_step_interval = 200 #inf
...
def setup(
    ...
    global out_dir
    hp_name = f'10m_mdm-{args.model}M-{args.flops}'
    out_dir = Path('workdir/pretrain') / hp_name
    wandb_logger = WandbLogger(name=f'{hp_name}-mc', save_dir=out_dir, project='pretrain-c4')
...
def main(fabric, train_data_dir, val_data_dir, resume):
    ...
    assert config.block_size == args.block_size, "block_size in config does not match the argument"
...
if __name__ == "__main__":
    ...
    setup(
        devices = args.n_gpu,
        train_data_dir=Path("data/c4_10m"),
        val_data_dir=Path("data/c4_validation"),
    )

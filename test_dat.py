...
from lit_gpt.datmodel import DualStreamGPT
...
CKPT_PATH = "workdir/finetune/dat-336M-reverse/iter-009600-ckpt.pth"
PARAM=336
...
model = DualStreamGPT(config).to(DEVICE)
...
# ans = ans[:6]+'D'+ans[7:]
question_ids = tokenizer([q], padding="max_length", max_length=256, truncation=False, return_tensors="pt")['input_ids'].to(DEVICE)
...
resp_ids = torch.full((1,16), 0, dtype=question_ids.dtype, device=question_ids.device)
